---
title: "[Placeholder]Summary - update later"
author:
- Xiao Chen
...
Or using reticulate and running through engine pointed by reticulate..

```{r eval=FALSE}
library(reticulate)
py_install("openai")
py_install("tiktoken")
py_install("pinecone-client") # getting some error to run this from aarm architecture and also from amd linux 
py_install("lanchain")
```
...
file = open(f'{BASEPATH}api_keys')
lines = file.readlines()

openai_key = lines[1].strip()
pinecone_key = lines[3].strip()
pinecone_env = lines[4].strip()

OPENAI_API_KEY = os.environ.get('OPENAI_API_KEY', openai_key)
...
index_name ="anly545" # put in the name of your pinecone index here
